...
// as Server Actions. These server functions can then be imported and used in Client 
// and Server components. Any functions included in this file that are not used will 
// be automatically removed from the final application bundle.
...
// consider using the entries() method with JavaScript's Object.fromEntries().
...
export type State = {
    ...
    message?: string | null;
    //add data for mantaining form state on validation errors
    data?: {
        customerId?: string;
        amount?: string;
        status?: string;
...
export async function createInvoice(prevState: State, formData: FormData) {
    //Estrai i dati prima della validazione
    const rawData = {
    ...
    if (!validatedFields.success) {
        return {
            ...
            data: rawData, // ✅ Ritorna i dati inseriti
        };
    }
